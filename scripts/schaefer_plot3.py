...
if __name__ == '__main__':
    ...
    for parc in np.arange(100, 400, 100):
        ...
        for pctComp in np.arange(0, 1.02, 0.02):
            all_FC = np.zeros((nFCs, parc + 13, parc + 13))
            for i in np.arange(0, nFCs):
                all_FC[i] = utri2mat(reordered_FCs[i])
            print(f'Testing {pctComp} components...')
            all_FC = pca_recon(all_FC, pctComp)
            ...
            print(acc)
            accuracies[f"{pctComp}_{parc}"] = acc

    a_file = open(f"../results/schaefer_plot3.csv", "w")
